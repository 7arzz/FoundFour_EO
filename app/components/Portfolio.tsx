...
const portfolioItems = [
  ...
  {
    id: 7,
    title: "Gourmet Pastry",
    category: "Culinary Campaign",
    img: "/images/product.jpg",
  },
  {
    id: 8,
  ...
  {
    id: 10,
    title: "Velvet Jewelry",
    category: "High-End Detail",
    img: "/images/product.jpg",
  },
];
...
const Portfolio = () => {
  ...
  const animate = () => {
    if (!isDragging) {
      speed.current.current +=
        (speed.current.target - speed.current.current) * 0.05;
  ...
  const onStart = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    dragStart.current = {
      x,
      y,
      lastX: rotation.current.y,
      lastY: rotation.current.x,
    };
  };
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent | TouchEvent) => {
      ...
      rotation.current.x = Math.max(
        -40,
        Math.min(40, dragStart.current.lastY - deltaY * 0.3),
      );
    };
  ...
  return (
    ...
    >
              ...
              style={
                {
                  ...
                  transitionDelay: isVisible
                    ? `${idx * 0.1}s`
                    : `${(portfolioItems.length - idx) * 0.05}s`,
      ...
      <div className="instructions" style={{ opacity: isVisible ? 0.6 : 0 }}>
        Swipe to Rotate • <br /> Click to Expand
      </div>
      ...
      <div className={`modal-3d ${modalData ? "active" : ""}`}>
        {modalData && (
          <>
            <div className="close-modal-3d" onClick={() => setModalData(null)}>
              &times;
            </div>
            <div className="modal-content-3d">
              <Image
                ...
                width={600}
                height={600}
                className="modal-img-3d"
                style={{ objectFit: "cover" }}
              />
              <div className="modal-info-3d">
                <h2>{modalData.title}</h2>
                <p>
                  This project represents our commitment to excellence in visual
                  storytelling. By combining high-end styling with technical
                  precision, we create imagery that resonates with audiences and
